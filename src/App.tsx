...
function App() {
  ...
  const [serverUrl,setServerUrl]=useState(()=>localStorage.getItem('reson.serverUrl')??'https://api.reson-music.com');
  ...
  useEffect(()=>{
    ...
    if(!saved || saved==='http://127.0.0.1:4782' || saved==='http://0.0.0.0:4782' || saved==='http://192.168.150.188:4782'){
      setServerUrl('https://api.reson-music.com');
    }
  },[]);
...
const addTrackToPlaylist = (playlistId: string, trackId: string) => {
  ...
  const backupThisDevice=async()=>{
    ...
    try{
      ...
      const picture=(await imageToPortableData(profilePicture))||socialUser?.avatarUrl||'';
      const banner=(await imageToPortableData(profileBanner))||socialUser?.bannerUrl||'';
        ...
        <header className="topbar">
          ...
          <div className="account-actions">
            ...
            <div className="topbar-popover-wrap"><button className={`profile ${profileOpen?'active':''}`} onClick={()=>{setProfileOpen(v=>!v);setNoticesOpen(false)}}><span className="profile-avatar-small">{profilePicture?<img src={resonImageSrc(profilePicture)} alt="" style={cropImageStyle(avatarCrop)}/>:profileInitials}</span><Icon name="chevronDown"/></button>
              {profileOpen&&<div className="reson-popover profile-popover" onMouseDown={(e)=>e.stopPropagation()} onClick={(e)=>e.stopPropagation()}><div className="profile-card"><span className="profile-avatar-large">{profilePicture?<img src={resonImageSrc(profilePicture)} alt="" style={cropImageStyle(avatarCrop)}/>:profileInitials}</span><div><strong>{profileName}</strong><small>Local Reson profile</small></div></div><div className="profile-mini-stats"><span><b>{formatListenTime(totalListenSeconds)}</b> listened</span><span><b>{likedIds.length}</b> liked</span></div><button className="profile-primary-action" onClick={(e)=>{e.stopPropagation();setProfileOpen(false);setNoticesOpen(false);setActive('profile')}}><Icon name="artist"/>View Profile</button><button onClick={(e)=>{e.stopPropagation();setProfileOpen(false);setNoticesOpen(false);setActive('profile')}}><Icon name="album"/>Customize profile</button><button onClick={()=>{setActive('stats');setProfileOpen(false)}}><Icon name="stats"/>Listening stats</button><button onClick={()=>{setActive('settings');setProfileOpen(false)}}><Icon name="settings"/>Settings</button><div className="profile-divider"/><div className="profile-about"><strong>Reson</strong><span>v0.26.6 · Local music player</span></div></div>}
            </div>
          </div>
        </header>
...
function SettingsPage({volume,setVolume,settings,setSettings,folder,onChangeFolder,background,setBackground,onChooseBackground,diagnostics,updater,onCheckUpdate,onInstallUpdate}:{volume:number;setVolume:(v:number)=>void;settings:{crossfade:number;normalization:boolean;preamp:number;bass:number;mids:number;treble:number;gapless:boolean};setSettings:React.Dispatch<React.SetStateAction<{crossfade:number;normalization:boolean;preamp:number;bass:number;mids:number;treble:number;gapless:boolean}>>;folder:string;onChangeFolder:()=>void;background:AppBackgroundSettings;setBackground:React.Dispatch<React.SetStateAction<AppBackgroundSettings>>;onChooseBackground:()=>void;diagnostics:{trackCount:number;queueSize:number;currentWaveformSamples:number;lastScanMs:number|null};updater:{checking:boolean;available:boolean;version:string;notes:string;installing:boolean;progress:number|null;message:string;checkedAt:number|null};onCheckUpdate:()=>void;onInstallUpdate:()=>void}) {
  ...
  return <div className="settings-page"><div className="page-heading"><p>RESON</p><h1>Settings</h1><span>Tune playback and make Reson look and sound the way you want.</span></div>
    <section className={`settings-card update-settings-card ${updater.available?'has-update':''}`}>
      <div className="settings-card-head"><div><span className="settings-icon"><Icon name="refresh"/></span><div><h2>Reson Updates</h2><p>Signed updates are delivered through the official Reson GitHub releases.</p></div></div><div className="update-version-stack"><span className="update-test-badge"></span><span className="update-version-pill">v0.26.6</span></div></div>
...
function FriendsPage({serverUrl,setServerUrl,token,user,friends,requests,leaderboard,metric,status,message,onLogin,onLogout,onRefresh,onRequest,onRespond,onMetric,cloudBackupUpdatedAt,cloudSyncBusy,onBackup,onRestore,localProfilePicture}:{serverUrl:string;setServerUrl:(v:string)=>void;token:string;user:SocialUser|null;friends:SocialUser[];requests:FriendRequest[];leaderboard:LeaderboardEntry[];metric:'total'|'weekly'|'songs';status:'idle'|'connecting'|'online'|'offline';message:string;onLogin:(u:string,p:string,r:boolean)=>Promise<void>;onLogout:()=>void;onRefresh:()=>void;onRequest:(u:string)=>Promise<void>;onRespond:(id:number,a:boolean)=>Promise<void>;onMetric:(m:'total'|'weekly'|'songs')=>void;cloudBackupUpdatedAt:number|null;cloudSyncBusy:boolean;onBackup:()=>void;onRestore:()=>void;localProfilePicture:string}) {
  ...
  if(!token){
    return <div className="friends-page page-enter">
      ...
      <section className="social-login-card">
        <div className="social-server-row"><div><strong>Server</strong><small>Your server laptop's address, including port 4782.</small></div><input value={serverUrl} onChange={e=>setServerUrl(e.target.value)} placeholder="https://api.reson-music.com"/></div>
  ...
  return <div className="friends-page page-enter">
    ...
    <section className="social-me-card"><div><span className="social-avatar">{user?.avatarUrl?<img src={resonImageSrc(user.avatarUrl)} alt=""/>:(user?.displayName||user?.username||'R').slice(0,1).toUpperCase()}</span><div><strong>{user?.displayName||'Reson user'}</strong><small>@{user?.username||'—'}</small></div></div><div className="social-me-actions"><button onClick={onRefresh}>Refresh</button><button onClick={onLogout}>Sign out</button></div></section>
    {message&&<p className="social-message">{message}</p>}
    ...
    <section className="social-card leaderboard-card-large">
      ...
      {leaderboard.length?<div className="leaderboard-list-large">{leaderboard.map(entry=>{
        ...
        return <div className={`leaderboard-row-large rank-${entry.rank}`} key={entry.user.id}>
          ...
          <span className="social-avatar leaderboard-avatar">{(entry.user.avatarUrl||(entry.user.id===user?.id?localProfilePicture:''))?<img src={resonImageSrc(entry.user.avatarUrl||(entry.user.id===user?.id?localProfilePicture:''))} alt=""/>:entry.user.displayName.slice(0,1).toUpperCase()}</span>
